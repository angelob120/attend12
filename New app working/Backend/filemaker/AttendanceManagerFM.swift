...
// MARK: - Attendance Manager for FileMaker
class AttendanceManagerFM: ObservableObject {
    // Singleton instance
    static let shared = AttendanceManagerFM()
    
    // Published properties
    @Published var attendanceRecords: [AttendanceRecordFM] = []
    @Published var isLoading = false
    @Published var error: String?
    @Published var currentUserAttendance: AttendanceRecordFM?
    ...
    func fetchAttendanceRecords(for menteeID: UUID) async {
        ...
        do {
            let records = try await fileMakerService.fetchAttendanceRecords(for: menteeID)
    ...
    func fetchAttendanceRecords(from startDate: Date, to endDate: Date) async {
        ...
        do {
            let records = try await fileMakerService.fetchAttendanceRecords(from: startDate, to: endDate)
    ...
    func clockIn(menteeID: UUID, location: CLLocation? = nil) async -> Bool {
        ...
        do {
            ...
            let attendanceRecord = AttendanceRecordFM(
                menteeID: menteeID,
                date: now,
                clockInTime: now,
            ...
            // Save the record to FileMaker
            let savedRecord = try await fileMakerService.saveAttendance(attendanceRecord)
    ...
    func clockOut() async -> Bool {
        ...
        do {
            ...
            updatedAttendance.clockOutTime = Date()
            ...
            if currentAttendance.clockInTime.timeIntervalSince(expectedClockIn) > 15 * 60 {
                updatedAttendance.status = .tardy
            }
            
            // Save the updated record
            let updatedRecord = try await fileMakerService.updateAttendance(updatedAttendance)
    ...
    func markAbsent(menteeID: UUID, date: Date) async -> Bool {
        ...
        do {
            ...
            let absentRecord = AttendanceRecordFM(
                menteeID: menteeID,
                date: startOfDay,
            ...
            // Save the record to FileMaker
            let savedRecord = try await fileMakerService.saveAttendance(absentRecord)
    ...
    func updateAttendance(record: AttendanceRecordFM) async -> Bool {
        guard !isLoading else { return false }
        ...
        do {
            let updatedRecord = try await fileMakerService.updateAttendance(record)
    ...
    func deleteAttendance(record: AttendanceRecordFM) async -> Bool {
        guard !isLoading else { return false }
        ...
        do {
            try await fileMakerService.deleteAttendance(record)
    ...
    func getAttendanceRecord(for menteeID: UUID, on date: Date) -> AttendanceRecordFM? {
        let calendar = Calendar.current
    ...
    /// Convert AttendanceRecordFM to AppAttendanceStatus
    func convertToAppStatus(status: AttendanceStatusFM) -> AppAttendanceStatus {
        switch status {
        case .present:
            return .present
        case .absent:
